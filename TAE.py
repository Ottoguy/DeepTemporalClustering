...
from keras.layers import Input, Conv1D, LeakyReLU, MaxPool1D, Bidirectional, TimeDistributed, Dense, Reshape
...
def temporal_autoencoder(input_dim, timesteps, n_filters=50, kernel_size=10, strides=1, pool_size=10, n_units=[50, 1]):
    ...
    encoded = Bidirectional(n_units[0], return_sequences=True, merge_mode='sum')(encoded)
    encoded = LeakyReLU()(encoded)
    encoded = Bidirectional(n_units[1], return_sequences=True, merge_mode='sum')(encoded)
...
def temporal_autoencoder_v2(input_dim, timesteps, n_filters=50, kernel_size=10, strides=1, pool_size=10, n_units=[50, 1]):
    ...
    encoded = LeakyReLU()(encoded)
    encoded = MaxPool1D(pool_size)(encoded)
    encoded = Bidirectional(n_units[0], return_sequences=True, merge_mode='concat')(encoded)
    encoded = LeakyReLU()(encoded)
    encoded = Bidirectional(n_units[1], return_sequences=True, merge_mode='concat')(encoded)
    encoded = LeakyReLU(name='latent')(encoded)
